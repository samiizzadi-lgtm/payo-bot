...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not path:
            await query.edit_message_text(
                (
                    ...
                    else "📈 هنوز هزینه‌ای برای نمودار ثبت نشده."
                ),
                reply_markup=reports_menu(user_id),
            )
            return 0
    ...
    if data.startswith("goal_view:"):
        ...
        await query.edit_message_text(
            f"🎯 {goal['name']}\n\n"
            f"💰 {format_money(user_id, goal['current_amount'])} / "
            f"{format_money(user_id, goal['target_amount'])}",
            reply_markup=goals_menu(user_id, goals),
        )
        return 0
...
async def amount_handler(
    ...
    transaction_type = context.user_data.get(
        "transaction_type",
        "expense",
    )
