...
from keyboards import main_menu, expense_categories, income_categories
from database import add_transaction, get_financial_summary
from i18n import get_language, t, format_money
from charts import create_expense_chart
from radar import get_radar_report
from memory import get_memory_report
from reactions import react_to_message, get_expense_reaction, get_income_reaction
...
async def expense_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) >= 2:
        ...
        await react_to_message(update.message, get_expense_reaction(amount))
        return
...
async def income_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) >= 2:
        ...
        await react_to_message(update.message, get_income_reaction(amount))
        return
...
async def goals_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=goals_menu(user_id, get_user_goals(user_id)),
    )


async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    s = get_financial_summary(user_id)
    status = t(user_id, "positive") if s["balance"] > 0 else t(user_id, "neutral") if s["balance"] == 0 else t(user_id, "attention")
    await update.message.reply_text(
        f"{t(user_id, 'financial_report')}\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"{t(user_id, 'income_label')}: {format_money(user_id, s['income'])}\n"
        f"{t(user_id, 'expense_label')}: {format_money(user_id, s['expense'])}\n"
        f"{t(user_id, 'balance_label')}: {format_money(user_id, s['balance'])}\n\n"
        f"{t(user_id, 'transaction_count')}: {s['transaction_count']}\n"
        f"📌 {status}",
        reply_markup=main_menu(user_id),
    )
...
async def chart_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not path:
        await update.message.reply_text(
            "📈 No expenses are available for a chart yet." if _en(user_id)
    ...
    with open(path, "rb") as photo:
        await update.message.reply_photo(
            photo=photo,
            caption="📈 Payo Expense Chart" if _en(user_id) else "📈 نمودار هزینه‌های Payo",
        )
...
def main():
    ...
    application.add_handler(
        CallbackQueryHandler(ai_handler, pattern=r"^(ai_|technical_)"),
        group=0,
    )
    ...
    conversation = ConversationHandler(
        ...
        states={
            ...
            GOAL_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, goal_name_handler),
            ],
            GOAL_AMOUNT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, goal_amount_handler),
            ],
        },
